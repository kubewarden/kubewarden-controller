...
func TestDeletePendingClusterAdmissionPolicies(t *testing.T) {
	...
	validationPolicy := policiesv1alpha2.ClusterAdmissionPolicy{
		ObjectMeta: metav1.ObjectMeta{DeletionTimestamp: &metav1.Time{Time: time.Now()}, Name: admissionPolicyName, Finalizers: []string{"kubernetes"}},
	...
	mutatingPolicy := policiesv1alpha2.ClusterAdmissionPolicy{
		ObjectMeta: metav1.ObjectMeta{DeletionTimestamp: &metav1.Time{Time: time.Now()}, Name: mutatingPolicyName, Finalizers: []string{"kubernetes"}},
	...
	if policy.Finalizers != nil {
		t.Errorf("validating policy finalizers should be nil, but found %s", policy.Finalizers)
	}
	policy = &policiesv1alpha2.ClusterAdmissionPolicy{}
	err = r.Client.Get(context.Background(), client.ObjectKey{
		Name: mutatingPolicyName,
	...
	if policy.Finalizers != nil {
		t.Errorf("mutating policy finalizers should be nil, but found %s", policy.Finalizers)
	}
}
